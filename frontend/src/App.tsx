...
type DecisionCall = { id: string; stage: string; mode: string; status: string; requested_model: string; resolved_model: string; confidence: number; latency_ms: number; cached: boolean; fallback_used: boolean };
type RuntimeSettings = { llm_configured: boolean; smtp_configured: boolean; jev_enabled: boolean; jev_configured: boolean; jev_shadow_mode: boolean; jev_model: string; jev_auto_threshold: number; source_daily_request_budget: number; source_usage_total: number; sources: SourceOption[] };
...
export default function App() {
  ...
  const [decisions, setDecisions] = useState<DecisionCall[]>([]);
  ...
  const loadRunData = async (runId: string) => {
    const [currentEvents, currentArtifacts, currentPapers, currentDecisions] = await Promise.all([
      request<RunEvent[]>(`/runs/${runId}/events`),
      request<Artifact[]>(`/runs/${runId}/artifacts`),
      request<Paper[]>(`/runs/${runId}/papers`),
      request<DecisionCall[]>(`/runs/${runId}/decisions`),
    ]);
    setEvents(currentEvents); setArtifacts(currentArtifacts); setPapers(currentPapers); setDecisions(currentDecisions);
  };
  const openTask = async (task: Task) => {
    setError(""); setSelectedTask(task); setRun(null); setPapers([]); setEvents([]); setArtifacts([]); setDecisions([]);
  ...
  const startTask = async () => {
    ...
    setSelectedTask(task); setRun(createdRun); setPapers([]); setEvents([]); setArtifacts([]); setDecisions([]); await loadTasks();
  };
  ...
  const runSubscription = async (subscription: Subscription) => {
    ...
    setSelectedTask(task); setRun(createdRun); setPapers([]); setEvents([]); setArtifacts([]); setDecisions([]); setActiveView("workspace"); await loadDeliveries();
  };
  ...
  const averageDecisionConfidence = decisions.length ? decisions.reduce((sum, item) => sum + item.confidence, 0) / decisions.length : 0;
  ...
  return <div className="app-shell">
    ...
    </nav><div className="sidebar-foot"><CircleHelp size={16} /> <span>本地优先 · V2</span></div></aside>
    <main className="main-content"><header className="topbar"><div><p className="eyebrow">RESEARCH WORKSPACE</p><h1>{activeView === "workspace" ? "文献发现工作台" : activeView === "subscriptions" ? "每日文献订阅" : activeView === "prompts" ? "Prompt 版本" : "连接设置"}</h1></div><div className="connection"><span className="status-dot" />本地服务已连接</div></header>
      ...
      {activeView === "workspace" && <>
        ...
        <section className="workspace-section"><div className="section-toolbar"><div><span className="section-index">03</span><h2>结果</h2></div><div className="toolbar-meta"><span>{run ? statusLabel : "等待任务"}</span><span>{papers.length} papers</span><span>{reviewed} 已审核</span></div></div>
          ...
          {run && <div className="decision-panel"><strong>Jev 决策审计 · {runtimeSettings?.jev_configured ? (runtimeSettings.jev_shadow_mode ? "影子模式" : "主动模式") : "未启用"}</strong><div className="decision-metrics"><span><b>{decisions.length}</b> 次调用</span><span><b>{Math.round(averageDecisionConfidence * 100)}%</b> 平均置信度</span><span><b>{decisions.filter((item) => item.cached).length}</b> 缓存</span><span><b>{decisions.filter((item) => item.fallback_used).length}</b> 回退</span></div></div>}
      ...
      {activeView === "settings" && <section className="simple-section"><div className="section-toolbar"><div><span className="section-index">06</span><h2>连接设置</h2></div></div><div className="settings-list"><div><strong>模型提供商 · {runtimeSettings?.llm_configured ? "已配置" : "未配置"}</strong><span>通过后端 .env 配置 OpenAI-compatible API。</span></div><div><strong>Jev · {runtimeSettings?.jev_configured ? (runtimeSettings.jev_shadow_mode ? "影子模式" : "主动模式") : "未启用"}</strong><span>{runtimeSettings?.jev_model || "jev-1.13.0"}；自动阈值 {Math.round((runtimeSettings?.jev_auto_threshold || .82) * 100)}%。</span></div><div><strong>SMTP · {runtimeSettings?.smtp_configured ? "已配置" : "未配置"}</strong><span>密码只从环境变量读取。</span></div><div><strong>文献来源与预算</strong><span>Semantic Scholar、OpenAlex、Crossref、arXiv、PubMed；今日 {runtimeSettings?.source_usage_total || 0}/{runtimeSettings?.source_daily_request_budget || 200} 次。</span></div></div></section>}
    </main>
  </div>;
}
